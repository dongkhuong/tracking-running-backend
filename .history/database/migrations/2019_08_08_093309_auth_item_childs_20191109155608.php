...
class AuthItemChilds extends Migration
{
	...
	public function up()
	{
		Schema::create('auth_item_childs', function (Blueprint $table) {
			...
			$table->foreign('parent')->references('name')->on('auth_items')->onDelete('cascade')->onUpdate('cascade');
			$table->foreign('child')->references('name')->on('auth_items')->onDelete('cascade')->onUpdate('cascade');
		});
	}
