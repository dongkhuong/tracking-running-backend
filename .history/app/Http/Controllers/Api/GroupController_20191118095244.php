...
class GroupController extends MainController
{
    ...
    public function getAllGroupsOfUser() 
    {
        $temp = array();
        $cUsers = DB::table('group_user')->where('user_id', cuser()->id);
        if($cUsers->count() > 0) {
            foreach($cUsers->get() as $cUser){
                array_push($temp,$cUser->group_id);
            }
            $groups = Group::whereIn('id', $temp)->get();
            return $this->jsonOut([
                'business_code' => 1,
                'data' => $groups
            ]);
